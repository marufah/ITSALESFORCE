         ...
         if(p.Name.contains('SRV')||p.Name.contains('BTBB Profile')){
             ...
             if(Trigger.isInsert && CaseTriggerHandler.hasinsertRun != true){
              ...
              if(c.isclosed != true)
              {
                 ...
                 if(oldc.Status != c.Status && (c.Status == 'Pending User' || c.status =='Solution Suggested' || c.status=='Researching' || c.status == '3rd Party - Transact Use')){
                     system.debug('caserec1'+c.status);
                     caslist1.add(c);    
                 }     
             }
         }    
...
-       if((oldc.Last_Activity_Date_Time__c != c.Last_Activity_Date_Time__c) && (c.status == '3rd Party - Transact Use')){
-           deleteId.add(c.ID);
-           caslist1.add(c);
-       }               
-                      
...
+			if(oldc.Status != c.Status && (c.status == 'Repairs Complete' || c.status == 'Returned to Customer') && c.Case_record__c == 'RMA'){                
+                caslist1.add(c);
+            }              
+      	}
+	}
